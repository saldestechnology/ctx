...
Before any network work or filesystem mutation, ctx checks whether a package manager owns the
running executable and verifies that a direct-download install location is writable. Cargo,
Homebrew, Arch, Debian, RPM, and Scoop installations are refused with the appropriate upgrade
guidance. Path detection covers Cargo, Homebrew, and Scoop layouts; on Linux, local read-only
queries to pacman, dpkg, and RPM identify package-owned files.
...
installing a newer Debian/RPM package.
...
| 2 | Operational error (managed installation, network failure, checksum mismatch, install location not writable, unsupported platform, unknown version) |
